...
enum PrivilegeMode {
	...
	MODE_SUPERVISOR = 0x13,     //访管模式(PSR低5位为10011)，提供操作系统使用的一种保护模式
...
/*
异常发生时, ARM处理器会跳转到固定地址去执行异常处理程序, 这个固定的地址就是异常向量, 即异常处理程序入口地址
...
enum ExecutionVector {          //异常向量表（中断向量表）
	...
	BASE_UNDEF = 0x00000004,    //未定义指令：处理器无法识别指令的异常, 如果尝试执行不符合要求的指令会触发该异常
